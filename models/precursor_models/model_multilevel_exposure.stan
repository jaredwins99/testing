...
parameters {
  ...
  // Exposures
  real mu_gamma;                                       // ESTIMATE OF PRIMARY INTEREST: global mean exposure effect (center for Level 2)
  ...
  // Exposures
  real<lower=0> sigma_gamma_between;                   // SD OF INTEREST: exposure effects ACROSS restaurants (scale for Level 2)
  ...
  // Exposures
  vector[R] z_eta;                                     // ESTIMATES OF SECONDARY INTEREST: uncentered per-restaurant effects (deviates for Level 2)
  ...
  // Exposures
  real<lower=0> sigma_gamma_within;                    // SD OF INTEREST: exposure effects WITHIN a restaurant (scale for Level 1)
...
transformed parameters {
  ...
  // Use a local scope
  {
    ...
    eta = mu_gamma + sigma_gamma_between * z_eta;
    ...
    for (k in 1:K_exposure) { // Remember that this is the total exposures across restaurants
      gamma[k] = eta[expo_to_rest[k]] + sigma_gamma_within * z_gamma[k];
      beta[idx_exposure[k], expo_to_rest[k]] = gamma[k]; // Insert into beta
    }
  }
...
model {
  ...
  // Exposures
  z_eta ~ std_normal();  // Prior for non-centered deviates
