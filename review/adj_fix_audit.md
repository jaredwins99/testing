...
## Methods/Results text — moved

The paper-facing prose that used to live here (what a relative rate ratio is,
the baseline-exchangeability assumption, and its empirical check) now lives in
**`publication/METHODS_rrr.md`**, alongside the pipeline it describes rather
than inside this engineering audit.

The diagnostic behind it is `publication/scripts/exch_diag.R`, output in
`publication/exch_diag_baseline_gap.csv` (122 rows, one per subset pooled
estimate). Headline: median baseline gap 0.013, 81% below 0.10, 3/122 with a CI
excluding zero of which one reaches the figures (`t2_a4_its_t` dairy level,
gap −0.585).
