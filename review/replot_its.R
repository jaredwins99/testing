#!/usr/bin/env Rscript
# Replot ITS (A3) with correct scaling
...
its_outcomes <- list(meat = "meat_outcome", vegan = "vegan_outcome", vegetarian = "vegetarian_outcome",
                     nonvegan = "nonvegan_outcome", total = "total_outcome", chicken_fish = "chicken_fish_outcome")

for (cat_name in names(its_outcomes)) {
  cat(paste0("Processing ITS: ", cat_name, "\n"))
  ...
  for (rest in restaurants) {
    ...
    generate_plot(rest_df, rest, "its", cat_name, outcome_col, exp_col, output_dir)
  }
}
cat("ITS done!\n")
