...
LOG_OUTPUT_DIR_BASE <- paste0("forest_plots/z_log_and_overlay/t1", if (SORT_BY_MEAN) "_sorted" else "")
...
create_proportion_targeted_forest_restaurants <- function(log_scale = FALSE) {
  ...
  for (exp_type in c("count", "prop")) {
    model_path_a1 <- file.path("model_fits", DEFAULT_MODEL_PATH, "a1_proportion",
  ...
  if (!log_scale) {
    df_all <- df_all %>%
      mutate(
        # A2 Presence is binary (0/1) — use exp(.x) like Count. Restaurant rows
        # already come exp'd from exp_betas_95ci → pass through.
        across(c(q2.5, q97.5), ~ case_when(
          exposure_type == "Count" & estimate_type == "Pooled" ~ exp(.x),
...
create_its_targeted_forest_restaurants <- function(log_scale = FALSE) {
  ...
  model_path_total <- file.path("model_fits", total_model_path_name, "a3_its", "total")
