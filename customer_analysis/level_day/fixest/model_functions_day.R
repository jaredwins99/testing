...
aggregate_to_day <- function(data) {
  outcome_cols <- c("total_outcome", "vegan_outcome", "vegetarian_outcome",
                    "meat_outcome", "chicken_fish_outcome", "nonvegan_outcome",
                    "breakfast_outcome", "untextured_outcome")
  outcome_cols <- intersect(outcome_cols, colnames(data))

  id_cols <- c("customer_id", "location_id", "date")
  covariate_cols <- setdiff(colnames(data), c(id_cols, outcome_cols))

  data %>%
    group_by(across(all_of(id_cols))) %>%
    summarize(
      across(all_of(outcome_cols), sum),
...
fit_restaurant_model <- function(data, outcome, location_id, include_gender = TRUE, extra_price_predictor = NULL, train_frac = 0.9) {
  ...
  # Aggregate to day level BEFORE train/test split
  rest_data <- aggregate_to_day(rest_data)

  if (nrow(rest_data) == 0) {
    message(paste("No data after day-level aggregation for:", location_id))
    return(NULL)}
...
save_results <- function(results, outcome, output_dir = "customer_analysis/level_day/fixest/results") {
...
print_model_summary <- function(model_result) {
  ...
  cat("N observations (customer-days):", model_result$n_obs, "\n")
...
plot_predictions <- function(model_result, outcome_name, output_dir = "customer_analysis/level_day/fixest/pred_plots") {
