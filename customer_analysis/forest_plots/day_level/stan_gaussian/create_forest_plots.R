...
RESULTS_DIR   <- "model_fits/finalized_redone_trunc_cp/a5_customer_day"
...
extract_gender_interactions <- function(outcome_dir, outcome_name) {
  ...
  for (i in seq_len(nrow(gender_cols))) {
    ...
    # M = 2 + has_gendermale + has_genderfemale; indices 3/4 are the gender
    # interaction pooled coefficients when both are present.
