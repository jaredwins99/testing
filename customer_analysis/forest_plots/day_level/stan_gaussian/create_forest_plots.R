# Forest plots for Stan Gaussian INGARCH results (day-level, demeaned)
...
# effect sizes per restaurant (gamma) and global means (mu_gamma).
...
RESULTS_DIR   <- "model_fits/finalized_redone_trunc/customer_gaussian"
...
# -----------------------------------------------
#  Convert to effect sizes (identity link)
# -----------------------------------------------
# No exp() transform — raw coefficients on demeaned scale.
# Slope coefficients are per-year (date_num = date / 365.25).

to_effect_sizes_gaussian <- function(df) {
  df %>%
    mutate(
      effect      = estimate,
...
# -----------------------------------------------
#  Build forest plot (natural scale)
# -----------------------------------------------

build_forest_gaussian <- function(df, title, subtitle, color, outcome_levels, filename,
                                  width = 10, height = 8) {
...
# -----------------------------------------------
#  Create A5 Gaussian forest plots
# -----------------------------------------------

create_A5_gaussian_forest <- function() {
  cat("Creating A5 Stan Gaussian INGARCH forest plots (day-level, demeaned)...\n")
  ...
  if (!is.null(data$mu_gammas) && nrow(data$mu_gammas) > 0) {
    ...
    build_forest_gaussian(
      mu_df,
      title    = "A5: Stan Gaussian INGARCH - Global Mean (mu_gamma)",
      subtitle = "Hierarchical mean across restaurants | Points = posterior mean | Bars = 90% CrI",
      color    = "steelblue",
      outcome_levels = outcome_levels,
      filename = "A5_stan_gaussian_mu_gamma_forest",
      width = 10, height = max(4, length(outcome_levels) * 1.5))
  }
}
...
cat("========================================\n")
cat("Stan Gaussian INGARCH Forest Plots\n")
cat("(Day-Level, Demeaned)\n")
cat("========================================\n\n")

create_A5_gaussian_forest()
