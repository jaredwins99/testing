...
#
# Auto-discovers outcomes via list.dirs(). Some outcomes (e.g., meat,
# chicken_fish) may still be running on Sherlock -- directories without a
# summ.rds are silently skipped.
#
# Three facets:
...
RESULTS_DIR   <- "model_fits/finalized_redone_trunc_cp/t2_customer_gaussian_iid_day"
...
load_all_outcomes <- function() {
  ...
  for (outcome_dir in outcome_dirs) {
    ...
    if (!is.null(gammas)) {
      ...
      save_csv(gammas, outcome_name, "A5_t2_gaussian_iid_day_exposure")
    }
    ...
    if (!is.null(gender)) {
      ...
      save_csv(gender, outcome_name, "A5_t2_gaussian_iid_day_gender")
    }
  }
...
create_forest_plots <- function() {
  cat("Creating T2 A5 Stan Gaussian IID day-level forest plots...\n")
  ...
  build_forest_3col(
    all_df,
    title    = "T2 A5: Stan Gaussian IID (Day-Level, Demeaned)",
    subtitle = "Per-restaurant + pooled posteriors | Points = posterior mean | Bars = 95% CrI (q2.5-q97.5)",
    outcome_levels = outcome_levels,
    filename = "A5_t2_stan_gaussian_day_forest",
    width = 14, height = max(4, length(outcome_levels) * 6))
  ...
  if (nrow(global_df) > 0) {
    build_forest_3col(
      global_df,
      title    = "T2 A5: Day-Level Global Means (mu_gamma + mu_beta pooled gender)",
      subtitle = "Hierarchical means across restaurants | Points = posterior mean | Bars = 95% CrI",
      outcome_levels = outcome_levels,
      filename = "A5_t2_stan_gaussian_day_mu_gamma_forest",
      width = 14, height = max(4, length(outcome_levels) * 5))
  }
}
...
cat("========================================\n")
cat("T2 A5 Stan Gaussian IID Forest Plots\n")
