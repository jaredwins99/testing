...
mod <- cmdstan_model("model_multilevel_exposure.stan")
...
init_fn <- function(chain_id = 1) {
  ...
  if (K_exposure > 0) {
    init_fn$mu_gamma <- rnorm(1, 0, 0.1)
    init_fn$sigma_gamma_between <- abs(rnorm(1, 0, 0.5)) + 0.1
    init_fn$sigma_gamma_within <- abs(rnorm(1, 0, 0.5)) + 0.1
    init_fn$z_eta <- rnorm(R, 0, 1)
    init_fn$z_gamma <- rnorm(K_exposure, 0, 1)
  }
...
output_dir <- file.path("model_fits/empirical/multilevel_exposure", outcome_str)
