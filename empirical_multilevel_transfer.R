...
df_all_daily <- read_parquet("data/5_palate_data_parquet_modeling/all_locations_daily_weather_inflation.parquet") %>%
  ...
  left_join(before_after_details_true %>% dplyr::select(location_id, cross_over_date_num),
            by = "location_id") %>%
...
df_all_daily %>% filter(location_id == "SRQS8F7JWA9MZ") %>%
  ...
  geom_line() +
  labs(title = "Vegan Price Over Time for SRQS8F7JWA9MZ") +
  theme_minimal()
...
# <<< NEW: Manually create interaction terms >>>
# This creates new columns like 'exposure_VLZX7K2M9QD4T_1_slope'
interaction_predictors <- paste0(exposure_predictors, "_slope")
for(i in seq_along(exposure_predictors)){
  df_all_daily[[interaction_predictors[i]]] <- df_all_daily[[exposure_predictors[i]]] * df_all_daily[["date_num"]]
}

# Combine all predictors for the formula
...
#     3. Process Data  
# ──────────────────────────────────
# (Train/Test Split, Scaling, Matrix Creation)
...
all_numeric_cols <- df_all_daily %>%
  dplyr::select(where(is.numeric), # Covers excluding season
                -any_of(outcome), 
                -contains("_outcome"),
                -weekend,          # Exclude known binary vars
                -holiday_window
  ) %>%
  colnames()
...
print(paste("Identified", length(idx_beta_random), "random beta columns:",
...
# <<< NEW: Identify Exposure Indices
idx_exposure <- which(startsWith(model_colnames, "exposure_"))
K_exposure <- length(idx_exposure)
print(paste("Identified", K_exposure, "exposure columns in the design matrix."))


# <<< NEW: Beta Fixed Indices must now EXCLUDE exposures
idx_beta_fixed <- setdiff(1:J, c(idx_intercept, idx_beta_random, idx_exposure))
print(paste("Identified", length(idx_beta_fixed), "fixed beta columns:",
            paste(model_colnames[idx_beta_fixed], collapse=", ")))


# <<< NEW: Create the expo_to_rest mapping
# This mapping tells Stan which restaurant each exposure column belongs to.
if (K_exposure > 0) {
  expo_to_rest <- integer(K_exposure)
  for (k in 1:K_exposure) {
    col_idx <- idx_exposure[k]
    # Find which restaurants have a non-zero value in this exposure column
    active_restaurants <- unique(restaurant_id_train[X_train_all[, col_idx] != 0])
    
    if (length(active_restaurants) == 0) {
      warning(paste("Exposure column", model_colnames[col_idx], "is all zeros in the training data. Assigning to restaurant 1 as a placeholder."))
      expo_to_rest[k] <- 1 
    } else if (length(active_restaurants) > 1) {
      stop(paste("Exposure column", model_colnames[col_idx], "is active for multiple restaurants:", paste(active_restaurants, collapse=", "), ". This violates the model assumption that each exposure belongs to only one restaurant."))
    } else {
      expo_to_rest[k] <- active_restaurants
    }
  }
  print("Successfully created `expo_to_rest` mapping.")
} else {
  # Handle case with no exposure columns
  expo_to_rest <- integer(0)
}

# <<< NEW: Create the expo_to_param mapping >>>
# This new map links each exposure column to a parameter type (1=intercept, 2=slope)
M <- 2 # We have two parameter types: intercept and slope

if (K_exposure > 0) {
  # Get the names of all exposure-related columns from the model matrix
  exposure_colnames <- model_colnames[idx_exposure]
  
  # Assign 2 if the name ends in "_slope", otherwise assign 1
...
data_list <- list(
  ...
  # Column indices for random vs fixed effects within the design matrix
  ...
  idx_to_rest_train = restaurant_id_train,
  ...
  idx_to_rest_test = restaurant_id_test,
...
init_fn <- function(chain_id = 1) {
  ...
  # <<< NEW: Conditionally adds exposures
  init_list <- list(
...
output_dir <- file.path("model_fits/empirical/multilevel_transfer", outcome_str)
...
summ %>% filter(variable %>% str_detect("gamma")) %>% dplyr::select(variable,mean) %>% print(n=40)
summ %>% filter(variable %>% str_starts("beta"))  %>% print(n=100)
summ %>% filter(variable %>% str_detect("beta\\[(1[7-9]|2[0-9]),1\\]")) %>% print(n=40)
summ %>% filter(variable %>% str_detect("beta\\[(1[7-9]|2[0-9]),2\\]")) %>% print(n=40)
summ %>% filter(variable %>% str_detect("beta\\[(1[7-9]|2[0-9]),3\\]")) %>% print(n=40)
summ %>% filter(variable %>% str_detect("beta\\[(1[7-9]|2[0-9]),4\\]")) %>% print(n=40)
