...
#
# Output directories:
#   archive/forest_plots/professional_wide/t1_adj/   (6 PDFs)
#   archive/forest_plots/professional_wide/t2_adj/   (6 PDFs)
#
...
PROFESSIONAL_DIR_T1 <- "archive/forest_plots/professional_wide/t1_adj"
PROFESSIONAL_DIR_T2 <- "archive/forest_plots/professional_wide/t2_adj"
...
SOURCE_DIR_T1 <- "archive/forest_plots/total_adjusted/t1_sorted_recentered_wide"
SOURCE_DIR_T2 <- "archive/forest_plots/total_adjusted/t2_sorted_recentered_wide"
...
cat("\n[render_professional_wide] PRO_ONLY=", .only,
    " PRO_TIER=", .tier,
...
if (.run_recolored_t1) {
  cat("\n[render_professional_wide] Step 1a: render T1 A1–A4\n")
  source("publication/render/create_forest_plots_restaurants_chosen_recolored_adj.R",
         chdir = FALSE)
}

if (.run_recolored_t2) {
  cat("\n[render_professional_wide] Step 1b: render T2 A1–A4\n")
  source("publication/render/create_forest_plots_restaurants_chosen_recolored_adj_t2.R",
         chdir = FALSE)
}

if (.run_consolid) {
  cat("\n[render_professional_wide] Step 2: render A5/A6 day-level adj (both tiers)\n")
...
copy_pdfs <- function(stems, src_dir, dst_dir, tier_label) {
  copied  <- character(0)
  missing <- character(0)
  cat("\n[render_professional_wide] Copy PDFs ->", dst_dir, "\n")
  for (stem in stems) {
