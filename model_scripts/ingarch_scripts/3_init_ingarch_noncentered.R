## 3_init_ingarch_noncentered.R
##
## init_ingarch() as it stood at commit 3e52f8ad (2026-03-01, "truncated fits
## finished"). Emits the non-centered names (z_beta_intercept etc.) required by
## model_multilevel_transfer_truncated_noncentered.stan.
##
## Paired with that model for the 52 finalized_redone_trunc fits. These inits are
## not optional: sampling this model from random inits kills 100% of chains on
## neg_binomial_2_lpmf: Location parameter is inf.
##
## The function draws from rnorm() and has no internal seed -- it is reproducible
## only because the caller does set.seed(seed) first.
##
## Not on any live code path. Used only by refit_exact.R.
## Renamed from 3_init_ingarch.R; contents otherwise verbatim.
