# Forest plots for customer-level conditional Poisson analyses (A5, A6)
# Reads exposure results CSVs and creates static forest plots showing
# level change and slope change rate ratios per restaurant
# Generates both rate-ratio scale and log-scale versions

library(tidyverse)
library(ggplot2)

OUTPUT_DIR <- "customer_analysis/forest_plots/transaction_level/fixest"
OUTPUT_DIR_LOG <- "customer_analysis/forest_plots/transaction_level/fixest"
...
load_exposure_data <- function(files) {
  ...
  all <- all %>%
    ...
    mutate(
      ...
      # Extract exposure period number from term
...
to_rate_ratios <- function(df) {
  df %>%
    mutate(
      # Level change: exp(beta) = rate ratio
      # Slope change: exp(beta * 365) = annual rate ratio
      rr = case_when(
...
build_forest_log <- function(df, title, subtitle, color, outcome_levels, filename,
  ...
  # Beyond +/- 2.3 means >10x or <0.1x rate ratio, implausible for menu changes
...
# ─────────────────────────────────────
#  A5: Customer analysis (5 outcomes)
# ─────────────────────────────────────

create_A5_forest <- function() {
  cat("Creating A5 customer analysis forest plots...\n")

  files <- list.files("customer_analysis/transaction_level/fixest/results_exposures", pattern = "^A5_", full.names = TRUE)
  ...
  build_forest(
    df,
    title = "A5: Customer-Level Conditional Poisson Analysis",
...
# ─────────────────────────────────────
#  A6: Customer targeted analysis
# ─────────────────────────────────────

create_A6_forest <- function() {
  cat("Creating A6 customer targeted analysis forest plots...\n")

  files <- list.files("customer_analysis/transaction_level/fixest/results_exposures", pattern = "^A6_", full.names = TRUE)
  df <- load_exposure_data(files) %>% to_rate_ratios()
  ...
  build_forest(
    df,
    title = "A6: Customer-Level Targeted Categories Analysis",
    subtitle = "Rate ratios per restaurant | Level = immediate shift | Slope = annual trend change | Triangles = clipped",
    color = "darkorange",
    outcome_levels = a6_outcomes,
    filename = "A6_customer_targeted_forest",
    width = 10, height = 5)
...
cat("========================================\n")
cat("Customer Analysis Forest Plots\n")
cat("========================================\n\n")
