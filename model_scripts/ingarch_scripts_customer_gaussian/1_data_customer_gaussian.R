...
# ──────────────────────────────────
#  Compute Within-Customer Demeaned Outcomes
# ──────────────────────────────────
# Loads item-level transactions, aggregates to customer-day,
# filters customers with pre+post exposure observations,
# demeans within each customer, then aggregates to restaurant-day.

compute_demeaned_outcomes <- function(item_data_dir, outcome, restaurants_to_model) {

    outcome_col <- paste0(outcome, "_outcome")

    items <- read_parquet(item_data_dir) %>%
        filter(location_id %in% restaurants_to_model) %>%
        # Same date clipping as all other analyses
    ...
    # Identify exposure columns for this set of restaurants
    exposure_cols <- items %>%
        select(starts_with("exposure_"), -contains("slope")) %>%
        select(matches(paste(restaurants_to_model, collapse = "|"))) %>%
        colnames()

    # Aggregate items to customer x location x date
    cust_day <- items %>%
        group_by(customer_id, location_id, date) %>%
        summarize(
            outcome_val = sum(.data[[outcome_col]], na.rm = TRUE),
            any_exposure = as.integer(any(rowSums(across(all_of(exposure_cols))) > 0)),
            .groups = "drop")

    cat("  Customer-days before pre/post filter: ", nrow(cust_day), "\n")

    # Filter customers with both pre AND post exposure observations
    # (customers with no exposure variation provide no information)
    ...
    cat("  Customer-days after pre/post filter: ", nrow(cust_day),
        " from ", length(unique(cust_day$customer_id)), " customers\n")

    # Within-customer demeaning: subtract each customer's mean outcome
    cust_day <- cust_day %>%
        group_by(customer_id, location_id) %>%
        mutate(
            customer_mean = mean(outcome_val, na.rm = TRUE),
            outcome_demeaned = outcome_val - customer_mean) %>%
        ungroup()

    # Aggregate demeaned outcomes to restaurant-day level
    demeaned_restday <- cust_day %>%
        group_by(location_id, date) %>%
        summarize(
            demeaned_outcome = sum(outcome_demeaned, na.rm = TRUE),
            n_customers_demeaned = n(),
            .groups = "drop")

    cat("  Demeaned restaurant-days: ", nrow(demeaned_restday), "\n")
    cat("  Demeaned outcome range: [", round(min(demeaned_restday$demeaned_outcome), 3),
        ", ", round(max(demeaned_restday$demeaned_outcome), 3), "]\n")
...
prepare_data_customer_gaussian <- function(
    data_dir,
    item_data_dir,
    outcome,
    ...
    # ──────────────────────────────────
    #  1a. Compute Demeaned Outcomes from Item-Level Data
    # ──────────────────────────────────

    cat("Computing demeaned outcomes for '", outcome, "'...\n")
    demeaned_restday <- compute_demeaned_outcomes(item_data_dir, outcome, restaurants_to_model)

    # ──────────────────────────────────
    #  1b. Load Base Restaurant-Day Data
    # ──────────────────────────────────
    ...
    df_unscaled <- read_parquet(data_dir) %>%
        ...
        # Remove poor data boundaries (same as all analyses)
        filter(location_id != "2HRX9P6HKXA8V" | ('2019-01-01' < date & date < '2023-08-01')) %>%
        filter(location_id != "JHDN7CF1C03X5" | (date < '2023-06-01')) %>%
        filter(location_id != "EMBVNVD207CC6" | ('2016-06-01' < date & date < '2022-09-01')) %>%
        filter(location_id != "LBZEEFSBJNB3Z" | ('2021-09-01' < date & date < '2023-07-01')) %>%
    ...
    # ──────────────────────────────────
    #  1c. Replace Outcome with Demeaned Values
    # ──────────────────────────────────
    # Join demeaned outcomes BEFORE factor conversion (location_id is still char)

    df_unscaled <- df_unscaled %>%
        left_join(demeaned_restday, by = c("location_id", "date"))

    # Replace outcome column (NA = no qualifying customers that day, use 0)
    df_unscaled[[outcome_col]] <- ifelse(
        is.na(df_unscaled$demeaned_outcome), 0, df_unscaled$demeaned_outcome)
    df_unscaled <- df_unscaled %>% select(-demeaned_outcome, -any_of("n_customers_demeaned"))

    cat("Replaced '", outcome_col, "' with within-customer demeaned values\n",
        "  Range: [", round(min(df_unscaled[[outcome_col]]), 3),
        ", ", round(max(df_unscaled[[outcome_col]]), 3), "]\n")

    # ──────────────────────────────────
    #  1d. Continue Standard Pipeline
    # ──────────────────────────────────
