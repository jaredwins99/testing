...
prepare_data_transaction <- function(
    data_dir,
    outcome,
    ...
    # ──────────────────────────────────
    #   1b. Filter Customers Pre/Post
    # ──────────────────────────────────
    # Keep only customers with transactions both before AND after exposure
    # (customers with no exposure variation provide no information about gamma)
    ...
    matrix_list <- df_scaled %>%
      ...
      mutate(
        ...
        customer_id_loc = data %>% map(~ .x[["customer_id"]]),
      ...
      summarize(
        ...
        y = list(list_c(y_loc)),
        customer_id_vec = list(list_c(customer_id_loc)),
        N = sum(N_loc),
    ...
    {. %>% head() %>% print(); .} %>%
      ...
      pivot_wider(
        ...
        values_from = c(X, y, customer_id_vec, N, restaurant_id,
                        start_idx,
                        end_idx
                        ),
    ...
    #   4. Customer Indexing
    # ──────────────────────────────────
    # Compute contiguous customer blocks for the conditional Poisson likelihood

    compute_customer_index <- function(customer_id_vec, y_vec, restaurant_id_vec) {
        # customer_id_vec is already sorted by (restaurant, customer, date)
        # so same customer_id within same restaurant forms contiguous blocks
        cust_rle <- rle(customer_id_vec)
        C <- length(cust_rle$lengths)
        customer_end_idx <- cumsum(cust_rle$lengths)
        customer_start_idx <- c(1, head(customer_end_idx, -1) + 1)
        customer_to_rest <- restaurant_id_vec[customer_start_idx]
        n_i <- sapply(seq_len(C), function(c) sum(y_vec[customer_start_idx[c]:customer_end_idx[c]]))

        list(
            C = C,
            customer_start_idx = as.integer(customer_start_idx),
            customer_end_idx = as.integer(customer_end_idx),
            customer_to_rest = as.integer(customer_to_rest),
            n_i = as.integer(n_i))
    }

    train_cust <- compute_customer_index(
        matrix_list$customer_id_vec_train,
        matrix_list$y_train,
        matrix_list$restaurant_id_train)
    test_cust <- compute_customer_index(
        matrix_list$customer_id_vec_test,
        matrix_list$y_test,
        matrix_list$restaurant_id_test)

    matrix_list$C <- train_cust$C
    matrix_list$customer_start_idx <- train_cust$customer_start_idx
    matrix_list$customer_end_idx <- train_cust$customer_end_idx
    matrix_list$customer_to_rest <- train_cust$customer_to_rest
    matrix_list$n_i <- train_cust$n_i

    matrix_list$C_test <- test_cust$C
    matrix_list$customer_start_idx_test <- test_cust$customer_start_idx
    matrix_list$customer_end_idx_test <- test_cust$customer_end_idx
    matrix_list$customer_to_rest_test <- test_cust$customer_to_rest
    matrix_list$n_i_test <- test_cust$n_i

    print(paste("Train: N =", matrix_list$N_train, "customers =", train_cust$C))
    print(paste("Test: N =", matrix_list$N_test, "customers =", test_cust$C))
