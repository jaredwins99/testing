...
create_proportion_forest <- function() {
  ...
  for (outcome in outcomes) {
    for (exp_group in exposure_groups) {
      for (exp_type in exposure_types) {
        ...
        summ_path <- file.path(base_dir, "model_fits/finalized/proportion", 
  ...
  p <- ggplot(df, aes(x = mean, y = outcome)) +
    ...
    labs(
      ...
      subtitle = "Effect of MPBA exposure on food category proportions",
      x = "Effect Size (mu_gamma[1])",
      y = "Outcome") +
...
create_proportion_targeted_forest <- function() {
  ...
  for (i in seq_along(outcomes)) {
    ...
    for (exp_type in exposure_types) {
      ...
      summ_path <- file.path(base_dir, "model_fits/finalized/proportion_targeted", 
                             outcome, exposure, "summ.rds")
  ...
  p <- ggplot(df, aes(x = mean, y = outcome)) +
    ...
    labs(
      title = "A2: Proportion Targeted Analysis",
      subtitle = "Effect of targeted MPBA exposure on specific food categories",
      x = "Effect Size (mu_gamma[1])",
      y = "Outcome") +
    theme_minimal(base_size = 11) +
    theme(
...
create_its_forest <- function() {
  ...
  for (outcome in outcomes) {
    summ_path <- file.path(base_dir, "model_fits/finalized/its", outcome, "summ.rds")
  ...
  p <- ggplot(df, aes(x = mean, y = outcome)) +
    ...
    labs(
      ...
      subtitle = "Level and slope changes after MPBA introduction",
      x = "Effect Size (mu_gamma)",
      y = "Outcome") +
    scale_y_discrete(labels = function(x) format_label(x)) +
    theme_minimal(base_size = 11) +
    theme(
...
create_its_targeted_forest <- function() {
  ...
  for (outcome in outcomes) {
    summ_path <- file.path(base_dir, "model_fits/finalized/its_targeted", outcome, "summ.rds")
  ...
  p <- ggplot(df, aes(x = mean, y = outcome)) +
    ...
    facet_wrap(~ effect_type, ncol = 2) +
    labs(
      title = "A4: Interrupted Time Series Targeted Analysis",
      subtitle = "Level and slope changes for targeted food categories",
      x = "Effect Size (mu_gamma)",
      y = "Outcome") +
    scale_y_discrete(labels = function(x) format_label(x)) +
    theme_minimal(base_size = 11) +
