...
% drawn in the same style as exposure_outcome_diagram.tex.
%
...
\begin{tikzpicture}[
  ...
  minimatch/.style   = {-{Latex[length=1.0mm,width=0.9mm]}, dashed,
                        color=black!55, shorten >=10pt, shorten <=20pt},
...
% Grid cells (2 x 2)
% =============================================================
% Cells are big enough to hold a mini 3-column diagram inside.
\node[cell] (A1) at (0.0,   0.0) {};
\node[cell] (A2) at (12.5,  0.0) {};
\node[cell] (A3) at (0.0,  -5.0) {};
\node[cell] (A4) at (12.5, -5.0) {};

% -------------------------------------------------------------
% Macro: draw mini 3-column inside a cell.
% Args: cell anchor (north west corner), A# label, then the contents are
% built manually below per-cell because the row counts differ.
% -------------------------------------------------------------

% Within each cell:
%   - A# label top-left
%   - 3 mini sub-columns: Intensity (left), Item (middle), Outcome (right)
%   - sub-column headers in italic gray
%   - arrow or 1-to-1 matched arrows between mini-cols

% --- A1: no novelty, no specificity ---
% Intensity: Count, Proportion (2 rows)
% Item:      Analog-Modifiable, Vegan, Vegetarian (3 rows)
% Outcome:   Total, Nonvegan, Meat, Chicken/Fish, Vegetarian, Vegan (6 rows)
\node[alabel, anchor=north west] at ($(A1.north west)+(0.20,-0.15)$) {A1};

% sub-column x-offsets (relative to A1 north-west)
% c1 at 0.30 in, c2 at 4.30 in, c3 at 8.20 in (cm)
\coordinate (A1c1) at ($(A1.north west)+(0.30,-1.05)$);
\coordinate (A1c2) at ($(A1.north west)+(3.80,-1.05)$);
\coordinate (A1c3) at ($(A1.north west)+(7.80,-1.05)$);
...
\draw[miniarr] (A1c1box.east) to[out=0, in=180] ($(A1c2box.west)+(0,-0.25)$);
\draw[miniarr] (A1c2box.east) to[out=0, in=180] ($(A1c3box.west)+(0,-0.40)$);

% --- A2: no novelty, yes specificity ---
% Intensity: Presence (Binary), Count
% Item:      6 analog types
% Outcome:   6 counterpart sales (1-to-1 matched)
\node[alabel, anchor=north west] at ($(A2.north west)+(0.20,-0.15)$) {A2};

\coordinate (A2c1) at ($(A2.north west)+(0.30,-1.05)$);
\coordinate (A2c2) at ($(A2.north west)+(3.80,-1.05)$);
\coordinate (A2c3) at ($(A2.north west)+(7.80,-1.05)$);
...
\draw[miniarr] (A2c1box.east) to[out=0, in=180] ($(A2c2box.west)+(0,-0.85)$);
...
% Intensity: New Introduction
% Item:      Analog-Modifiable, Vegan, Vegetarian
% Outcome:   6 general sales
\node[alabel, anchor=north west] at ($(A3.north west)+(0.20,-0.15)$) {A3};

\coordinate (A3c1) at ($(A3.north west)+(0.30,-1.05)$);
\coordinate (A3c2) at ($(A3.north west)+(3.80,-1.05)$);
\coordinate (A3c3) at ($(A3.north west)+(7.80,-1.05)$);
...
\draw[miniarr] (A3c1box.east) to[out=0, in=180] ($(A3c2box.west)+(0,-0.40)$);
\draw[miniarr] (A3c2box.east) to[out=0, in=180] ($(A3c3box.west)+(0,-0.40)$);

% --- A4: yes novelty, yes specificity ---
% Intensity: New Introduction
% Item:      6 analog types
% Outcome:   6 counterpart sales (1-to-1)
\node[alabel, anchor=north west] at ($(A4.north west)+(0.20,-0.15)$) {A4};

\coordinate (A4c1) at ($(A4.north west)+(0.30,-1.05)$);
\coordinate (A4c2) at ($(A4.north west)+(3.80,-1.05)$);
\coordinate (A4c3) at ($(A4.north west)+(7.80,-1.05)$);
...
\draw[miniarr] (A4c1box.east) to[out=0, in=180] ($(A4c2box.west)+(0,-0.85)$);
...
% Labels shifted outward from row centers so the gap between them widens
% without changing font size. Each label is still centered on its row's
% vertical band -- we just push toward the band's outer edge.
\node[level, rotate=90, anchor=south]
  at ($(A1.west) + (-0.18, 0.75)$) {No: overall availability};
\node[level, rotate=90, anchor=south]
  at ($(A3.west) + (-0.18,-0.75)$) {Yes: new introductions};

\draw[arr]
  ($(A1.west) + (-1.05, 0.40)$) -- ($(A3.west) + (-1.05,-0.40)$);

% =============================================================
% Within-customer sensitivity (one-off, branching from A3 + A4)
% =============================================================
\node[ccell] (CC) at (5.2, -10.5) {};
