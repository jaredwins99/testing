...
% Both cross labels share a fixed y (just below the subhead row) so they
% align with each other regardless of where the arrows actually start/end.
\node[minicross, anchor=south]
  at ($(A1.north west)+(2.05,-1.00)$) {$\times$ crossed};
\node[minicross, anchor=south]
  at ($(A1.north west)+(5.80,-1.00)$) {$\times$ crossed};
...
\node[minicross, anchor=south]
  at ($(A2.north west)+(2.05,-1.00)$) {$\times$ crossed};

\foreach \i in {1,...,6} {
  \draw[minimatch] (A2c2r\i.east) -- (A2c3r\i.west);
}
\node[minicross, anchor=south]
  at ($(A2.north west)+(5.80,-1.00)$) {1-to-1 matched};
...
\node[minicross, anchor=south]
  at ($(A3.north west)+(2.05,-1.00)$) {$\times$ crossed};
\node[minicross, anchor=south]
  at ($(A3.north west)+(5.80,-1.00)$) {$\times$ crossed};
...
\node[minicross, anchor=south]
  at ($(A4.north west)+(2.05,-1.00)$) {$\times$ crossed};

\foreach \i in {1,...,6} {
  \draw[minimatch] (A4c2r\i.east) -- (A4c3r\i.west);
}
\node[minicross, anchor=south]
  at ($(A4.north west)+(5.55,-1.18)$) {1-to-1 matched};
