...
\node[desc,   anchor=north west, text width=75mm]
  ...
  {Analog availability (count, proportion) $\rightarrow$ meat sales};
...
\node[desc,   anchor=north west, text width=75mm]
  ...
  {New analog introductions $\rightarrow$ meat sales};
...
\draw[arr]
  ($(A1.north) + (0,0.85)$) -- ($(A2.north) + (0,0.85)$);
...
% Labels broken into two lines so each label spans a smaller vertical
% range on screen, allowing the arrow between them to be shorter and
% the gap to feel less stretched.
\node[level, rotate=90, anchor=south, align=center]
  at ($(A1.west) + (-0.18, 0.60)$) {No:\\overall availability};
\node[level, rotate=90, anchor=south, align=center]
  at ($(A3.west) + (-0.18,-0.60)$) {Yes:\\new introductions};

% vertical arrow next to "Novelty" header showing direction
\draw[arr]
  ($(A1.west) + (-1.05, 0.55)$) -- ($(A3.west) + (-1.05,-0.55)$);
