...
\begin{tikzpicture}[
  ...
  ccell/.style       = {draw=black!50, thick, dashed, rounded corners=2pt,
                        minimum width=75mm, minimum height=28mm,
...
\node[countline, anchor=south west] at ($(A1.south west)+(0.25,0.90)$)
...
\node[countline, anchor=south west] at ($(A2.south west)+(0.25,0.90)$)
...
\node[countline, anchor=south west] at ($(A3.south west)+(0.25,0.90)$)
...
\node[countline, anchor=south west] at ($(A4.south west)+(0.25,0.90)$)
...
% two vertical leaders land cleanly inside the corners.
\node[ccell] (CC) at (2.85, -8.4) {};

\node[clabel, anchor=north west] at ($(CC.north west)+(0.25,-0.20)$)
  {Customer-level sensitivity};
\node[desc, anchor=north west, text width=68mm]
  at ($(CC.north west)+(0.25,-0.78)$)
  {Re-estimate A3, A4 within visiting customers.};
\node[countline, anchor=south west] at ($(CC.south west)+(0.25,0.90)$)
