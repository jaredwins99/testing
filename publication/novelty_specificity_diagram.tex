...
% Labels pushed toward outer edges of their row to widen the visual
% gap between "No" and "Yes" without changing font size.
\node[level, rotate=90, anchor=south]
  at ($(A1.west) + (-0.18, 1.10)$) {No: overall availability};
\node[level, rotate=90, anchor=south]
  at ($(A3.west) + (-0.18,-1.10)$) {Yes: new introductions};

% vertical arrow next to "Novelty" header showing direction
\draw[arr]
  ($(A1.west) + (-1.05, 1.20)$) -- ($(A3.west) + (-1.05,-1.20)$);
