...
raw_dirs <- list_fit_dirs(FITS_ROOT)
...
keep_idx <- unlist(lapply(split(seq_along(raw_dirs), tails),
                          function(ix) ix[which.min(ranks[ix])]),
                   use.names = FALSE)
deduped  <- raw_dirs[sort(keep_idx)]
cat("Found", length(raw_dirs), "fit dirs; after cp2 > cp > trunc dedupe:",
    length(deduped), "\n")

pairs <- pair_fits(deduped)
