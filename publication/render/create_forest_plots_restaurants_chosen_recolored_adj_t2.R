        ...
        geom_point(data = df_restaurant,
                   aes(x = mean_disp, y = y_numeric, color = rest_color,
                       ...
                       text = paste0(
                         ...
                         "Percentage change: ", pub_pct_hover(mean_orig), "<br>",
                         "Relative Rate Ratio: ", signif(mean_orig, 3), "<br>",
...
create_proportion_targeted_forest_restaurants <- function(log_scale = FALSE) {
  ...
  # EMBVNVD207CC6 was removed from this outcome by re-fitting (see the
  # 'EMBVNVD A2 trim' re-fit); it no longer appears in breakfast_p, so the
  # old display-time drop-and-shrink is gone rather than left inert.
