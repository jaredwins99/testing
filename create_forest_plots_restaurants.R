...
extract_mu_gamma <- function(model_path, gamma_index = 1) {
  # Use 95% CI helper which computes q2.5 and q97.5 from samples
...
# Extract restaurant-level gammas from a model using 95% CI helpers
...
create_proportion_forest_restaurants <- function(log_scale = FALSE, model_run_path = "finalized_redone", output_dir_base = "forest_plots_restaurants_redone") {
  ...
  # Exponentiate parameters unless log_scale is TRUE
  if (!log_scale) {
    df_all <- df_all %>%
      mutate(
...
create_its_forest_restaurants <- function(log_scale = FALSE, model_run_path = "finalized_redone", output_dir_base = "forest_plots_restaurants_redone") {
  ...
  # Exponentiate pooled parameters only (restaurant already done), unless log_scale is TRUE
  if (!log_scale) {
...
create_its_targeted_forest_restaurants <- function(log_scale = FALSE, model_run_path = "finalized_redone", output_dir_base = "forest_plots_restaurants_redone") {
  ...
  # Exponentiate pooled parameters only, unless log_scale is TRUE
  if (!log_scale) {
    df_pooled_exp <- df_all %>%
      filter(estimate_type == "Pooled") %>%
      exp_params(col = "effect_type", slope_id = "Slope", unit = "year")
